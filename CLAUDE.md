...
- **次：Phase 2 ①「自分史チャット（対話モード）」** — 設問・素材庫・現草案を文脈にAIと往復し、成果を草案に反映。あわせて「添削ガイドラインのプロンプト注入」で“育てられる”基盤を入れる（Claude本体のFTはAPI非対応のため、プロンプト/few-shot/好み蓄積で実現）。
- Phase 2 ②：3通り生成。Phase 3：④企業提案（Web検索で根拠付け・"参考提案"に留める。LLM記憶だけの断定は知識カットオフ＆ハルシネーションで危険）。
