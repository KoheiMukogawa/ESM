...
- **設問の前後移動 = ✅**（2026-09-01）：設問カードのヘッダーを「設問文」→「設問 2 / 5」に、ツールバー（完了の隣）に `←` `→` を追加（`qNavState()`）。端では無効・ループなし・1問なら非表示。ドロワー化で「☰→設問」の2タップになっていたのを1タップに。**矢印を下に置くのは意図的**（次へ行きたくなるのは読み終えたページ下端にいるときなので、ヘッダーだと毎回上へ戻ることになる）。スクロール量での自動送り（TikTok風）は発見性とコスト比で見送り。
...
- **キーボードだけで動かす = ✅**（2026-09-12）：既定は本文にカーソル（`focusBody()`／起動時・設問選択・草案選択・検索ジャンプ時。**≤640px は無効**＝ソフトキーボード対策）。`Esc` で本文を抜けて**移動モード**（`body.nav-mode` で枠表示）、`↑↓` 企業（`coNavState()`）・`←→` 設問（`qNavState()`）・`Enter` で本文へ復帰。端で止まりループしない。**この2つは本来ぶつかる**（グローバル keydown は入力欄で早期 return するので、本文にフォーカスがあると素の矢印は発火しない）ためモードで分けた。`Tab` はブラウザ標準のフォーカス送りなので奪わない。`Esc` の処理は早期 return より**前**に置くこと（入力中でもモーダルを閉じるため）。`Ctrl+K` 検索ジャンプは本人判断で却下。
